...
const TokenInput = ({ selectType }) => {
    ...
    const getAmount = async () => {
        ...
        if (autoInputCondition_1) {
            try {
                console.log("case-1");
                console.log("INPUT: ", inputToken, "OUTPUT", outputToken);
                    ...
                    <button className='flex select-token align-center' onClick={() => setShowModal(true)} disabled={!account}>
                        {
                            (state.inputToken === null && selectType === INPUT) || (state.outputToken === null && selectType === OUTPUT) ? <span className='flex'>Select Token</span> :
                                selectType === INPUT ? <span className='flex align-center'><Image classname="flex selected-token-ico" alt={state.inputSymbol} src={`https://assets.trustwalletapp.com/blockchains/smartchain/assets/${state.inputToken}/logo.png`} fallbackSrc={Unknown} binanceCoin={state.inputSymbol === 'BNB' ? true : false}/><span className='flex'>{state.inputSymbol}</span></span> :
                                    <span className='flex align-center'><Image classname="flex selected-token-ico" alt={state.outputSymbol} src={`https://assets.trustwalletapp.com/blockchains/smartchain/assets/${state.outputToken}/logo.png`} fallbackSrc={Unknown} binanceCoin={state.outputSymbol === 'BNB' ? true : false}/><span className='flex'>{state.outputSymbol}</span></span>
                        }
                    </button>
                </div>
            </div>
            <SearchModal show={showModal} width="450px" height="600px" style={{ minWidth: "320px !important" }} effect="fadeInUp" onClickAway={() => setShowModal(false)} selectType={selectType} />
        </>
    )
}
