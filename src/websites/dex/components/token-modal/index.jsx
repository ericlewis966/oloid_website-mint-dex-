...
import { wbnb_address, oloid_address } from '../../configs/types';

import Unknown from '../../assets/images/unknown.png';
import Unknown1 from '../../assets/images/unknown2.png';
...
const SearchModal = ({ show, width, height, effect, style, selectType, onClickAway }) => {
    ...
    const OloidAddress = oloid_address;
        ...
        if (chainId === 56) {
            axios.get(`https://api.pancakeswap.info/api/v2/tokens/${address}`).then(res => {
                if (Object.keys(res.data.data).length > 4) {
                    ...
                    setTokens(tokens);
                    tokens.unshift({ address: oloid_address, detail: { name: 'Oloid', symbol: 'OLOID' } });
                    console.log(tokens);
                } else {
    ...
    const selectToken = async (token) => {
        ...
        if (account && (chainId === 56 || chainId === 97)) {
            ...
            const safeBalance = toSafeNumber(balance.toString(), decimals);
    ...
    const setBinanceCoin = async () => {
        ...
        const safeBalance = toSafeNumber(bnbBalance.toString(), 18);
                    ...
                    <div className='flex col justify-center token-list'>
                        {
                            tokens.map((item, key) => (
                                <div className={`flex coin-item align-center ${(selectType === INPUT && state.outputToken === item.address) || (selectType === OUTPUT && state.inputToken === item.address) ? 'coin-item-disabled' : ''}`} key={key} onClick={() => selectToken(item)}>
                                    <span className='flex asset-ico'><Image alt={item.detail.name} src={`https://assets.trustwalletapp.com/blockchains/smartchain/assets/${item.address}/logo.png`} fallbackSrc={item.address == OloidAddress ? Unknown : Unknown1} /></span>
